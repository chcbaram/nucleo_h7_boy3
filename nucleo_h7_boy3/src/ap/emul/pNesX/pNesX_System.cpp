...
/*                                                                   */
/*===================================================================*/
int pNesX_ReadRomFlash( uint8_t *p_data  )
{
  uint32_t f_index = 0;

  memcpy(&NesHeader, p_data, sizeof(NesHeader));
  f_index += sizeof(NesHeader);
  ...
  {
      printf("Read Trainer\r\n");
      memcpy(&SRAM[ 0x1000 ], &p_data[f_index], 512);
      f_index += 512;
  }
    ...
    /* Read ROM Image */
    f_index += 0x4000 * NesHeader.byRomSize;
    ...
    if ( NesHeader.byVRomSize > 0 )
    {
        ...
        /* Read VROM Image */
        f_index += 0x2000 * NesHeader.byVRomSize;
    }
 ...
 */
    ...
    if (millis()-fps_pre_time >= 1000)
    {
      fps_save = fps;
        ...
        printf("%d %d %d\r\n", fps, (int)(millis()-fps_pre_time), dmawait);
        fps = 0;
        fps_pre_time = millis();
    }
    dmawait = 0;
...
void pNesX_TransmitLinedata()
{
  ...
  */
  ...
  //gb.tft.drawBuffer(72*2, PPU_Scanline, pDrawData, 256, 0);
}
  ...
  *pdwSystem = ((pad->report[5] & 0x4) >> 2) |
  ...
  USBHost::poll();
    ...
    *pdwSystem = ((pad1 & 0x80) >> 7) |
    ...
    if (buttonGetPressed(_DEF_HW_BTN_HOME))
    {
      data |= (1<<2);
    }
    ...
    if (buttonGetPressed(_DEF_HW_BTN_X))
    {
      ApuAdjustVolume(-1);
    }
    if (buttonGetPressed(_DEF_HW_BTN_Y))
    {
      ApuAdjustVolume(+1);
    }
