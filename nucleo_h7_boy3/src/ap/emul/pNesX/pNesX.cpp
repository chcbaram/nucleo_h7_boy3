...
#if defined (__GNUC__)
asm(".section .rom_1_bin,\"a\";"
                         ".incbin \"../src/lib/rom/smario1.nes\";"
);
asm(".section .rom_2_bin,\"a\";"
                         ".incbin \"../src/lib/rom/1942.nes\";"
);

#endif


extern uint32_t rom_1_bin;
extern uint32_t rom_1_bin_end;
extern uint32_t rom_2_bin;
extern uint32_t rom_2_bin_end;
...
/* PPU RAM */
BYTE PPURAM[ PPURAM_SIZE ] __attribute__((section(".sram_d2")));
...
/*===================================================================*/
/*                                                                   */
/*                  pNesX_LoadFlash() : Load a cassette                   */
/*                                                                   */
/*===================================================================*/
int pNesX_LoadFlash( uint8_t *p_data )
{
/*
 *  Load a cassette
 *
 ...
 */
  ...
  // Read a ROM image in the memory
  if ( pNesX_ReadRomFlash( p_data ) < 0 )
    return -1;
 ...
 */
  ...
  if (index == 0)
  {
    if ( pNesX_LoadFlash( (uint8_t *)&rom_1_bin ) == 0 )
    {
  ...
  {
    pNesX_LoadFlash( (uint8_t *)&rom_2_bin );
  }
 ...
 */
  ...
  for (;;)
  {
    ...
    {
      pre_time = millis();
      error_fps = (60 - fps_save) / 2;

      out_fps = exec_cycle - error_fps;
      exec_cycle = constrain(out_fps, 0, 400);
      printf(".... %d %d %d\n", exec_cycle, fps_save, error_fps);
    }


    // Execute instructions
    K6502_Step( exec_cycle);
